	...
	twitterCallback: function (obj) {
		...
		for ( var i = 0; i < tweets.length && tweetCount < maxTweets; i++ ) {
			...
			if ( search || !makkintosshu.twitterStatuses.filterReplies || nonReply ) {
				...
				if ( !search )
				{
					statuses_html += ' <span class="when"><a href="http://twitter.com/' + tweets[i].user.screen_name + '/status/' + tweets[i].id_str + '">' + makkintosshu.twitterStatuses.relativeTime(makkintosshu.twitterStatuses.fixDate(tweets[i].created_at)) + "</a></span></li>\n";
				}
				else
				{
					statuses_html += ' <span class="when"><a href="http://twitter.com/' + tweets[i].from_user + '/status/' + tweets[i].id_str + '">' + makkintosshu.twitterStatuses.relativeTime(tweets[i].created_at) + "</a></span></li>\n";
				}
